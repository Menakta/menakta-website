...
export default function Hero() {
  return (
    <section className="relative min-h-screen bg-menakta-secondaryFirst overflow-hidden flex items-center justify-center">
      {/* Background gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-b from-menakta-secondaryFirst via-menakta-secondaryFirst to-transparent z-0" />

      {/* Content */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 className="font-tusker text-5xl sm:text-6xl md:text-7xl lg:text-8xl text-menakta-secondarySecond uppercase tracking-tight leading-none mb-6">
          Technology
          <span className="block text-menakta-primarySecond">
            Solution
          </span>
        </h1>

        <p className="font-clash text-menakta-secondarySecond/70 text-base sm:text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris.
        </p>

        <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
          <button className="font-clash px-8 py-4 bg-gradient-to-r from-menakta-primaryFirst to-menakta-primarySecond text-menakta-secondarySecond rounded-full text-sm uppercase tracking-wider hover:opacity-90 transition-opacity duration-300">
            Get Started
          </button>
          <button className="font-clash px-8 py-4 border border-menakta-secondarySecond/30 text-menakta-secondarySecond rounded-full text-sm uppercase tracking-wider hover:bg-menakta-secondarySecond/10 transition-all duration-300">
            Learn More
          </button>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-64 sm:h-80 md:h-96 overflow-hidden">
        {/* Wave 1 - Back */}
        <svg
